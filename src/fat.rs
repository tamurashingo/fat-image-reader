...
impl Region {
  pub fn new(image: &Vec<u8>) -> Self {
    let fat_start_sector = bootsector::reserved_sectors(image);
    let fat_sector_size = bootsector::sectors_per_fats(image) * bootsector::num_of_fats(image);
    let root_dir_start_sector = fat_start_sector + fat_sector_size;
    let root_dir_sector_size = (32 * bootsector::num_of_root_directory_entries(image) + bootsector::bytes_per_sector(image) - 1) / bootsector::bytes_per_sector(image);
    let data_start_sector = root_dir_start_sector + root_dir_sector_size;
    let data_sector_size = bootsector::total_sectors(image) - data_start_sector;
    let fat_type = fat_type(data_sector_size, bootsector::sectors_per_cluster(image));
    let sector_size = bootsector::bytes_per_sector(image);
